...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    ...
    html = html.replace('</style>', '.hosted-lock{position:fixed;z-index:9999;right:14px;bottom:14px;margin:0}.hosted-lock button{border:1px solid #fff;background:#242426;color:#fff;padding:8px 12px;font:11px Manrope,Arial,sans-serif;cursor:pointer}</style>').replace('</body>', `<form class="hosted-lock" method="post" action="${PROPOSAL_PATH}"><input type="hidden" name="action" value="logout"><button type="submit">Lock</button></form></body>`);
