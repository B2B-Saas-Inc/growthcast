...
The public blog is Astro-native and has no backend, admin system, or remote content dependency. `src/content.config.ts` validates Markdown/MDX frontmatter in `src/content/blog`. `/blog` renders image-led article cards with browser-side query and tag filtering suitable for static hosting; `/blog/[id]` preserves the MediaMixModel reference UX with breadcrumb and back navigation, author/read-time metadata, a boxed sticky table-of-contents/share rail that stacks on mobile, BreadcrumbList, and BlogPosting schema. Do not render the post thumbnail inside the article page. Generate each post's standalone abstract artwork with `scripts/generate-blog-shape.html`, then compose it with article title, author, and publication date using the safe-zone p5 template at `scripts/generate-blog-social.html`; only the finished card belongs in post `image` frontmatter for Open Graph metadata and `/blog` listings. `/rss.xml` and the Astro sitemap integration publish discovery feeds. Blog pages reuse the agency site's complete Company/Resources navigation and contact entry point, local Manrope/DM Mono fonts, and GrowthCast colors from the existing visual system.
