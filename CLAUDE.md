...
## Private client proposal route

`/clients/proposals/bluealpha` is an explicitly requested exception to the otherwise unauthenticated static site. The Vercel `api/bluealpha-proposal.ts` function checks an eight-digit PIN on the server and issues an eight-hour signed, Secure, HttpOnly, SameSite=Strict cookie. HTML and PDF requests both require this session. Responses are no-store/noindex and are not added to the public sitemap or analytics. The API route also enforces authorization if accessed directly.

Set server-only `BLUEALPHA_PIN_SHA256` (SHA-256 of the PIN) and `BLUEALPHA_SESSION_SECRET` (at least 32 random bytes) in Vercel Preview and Production. Rotating either revokes sessions. The PIN must never be committed or embedded in a client response. There is a bounded per-instance failed-attempt limiter; it is not a distributed rate limit.

The HTML and PDF are compressed and encrypted together with AES-256-GCM in `server-assets/bluealpha-proposal.enc`. Set the separate 32-byte hex `BLUEALPHA_CONTENT_KEY` in Vercel Preview and Production. The function decrypts only after session validation. The encrypted bundle can be tracked for repeatable deployments; plaintext client material and PINs must never be committed or put in `public/` or `dist/`. Vercel includes the encrypted file only in the function bundle. The static nginx image does not serve this Vercel-only route. Updating content requires re-encryption and deployment; changing the content key requires re-encryption. The automated API tests cover anonymous access, direct API/PDF protection, incorrect PINs, session expiry/tampering, CSRF, logout, missing configuration and per-instance throttling.
