...
3. Use GrowthCast as the agency and default model identity, while preserving the editable model name on Baseline for white-labelled document titles and export filenames. The default route is the GTM Engineering agency homepage; Why GrowthCast and How it works live at `/why-growthcast` and `/how-it-works`; the forecast product starts at `/resources/tools/forecast` under Resources > Tools > Forecast. Keep those pages distinct. Services has an index at `/services` and eight top-level service pages at `/services/*`; include Services in the main navigation.
...
- The public application has no accounts or database. Explicitly requested private BlueAlpha and PhotoShelter proposals use separate server-side PINs, encrypted bundles, and signed expiring HttpOnly cookies; see README for their isolated Vercel routes. The static site has one bounded Vercel `/api/lead` endpoint that validates agency leads, assigns a submission ID, and captures `lead_form_submitted` server-side in PostHog. It must never call Attio directly or expose server configuration through `VITE_` variables.
...
- Keep B2C revenue churn, logo churn, expansion, and downgrade as monthly rates. B2B has annual voluntary/delinquent logo churn, expansion, and downgrade, applied in forecast months 12, 24, and so on: churned accounts are `round(opening customers × annual logo churn)`, churned MRR is `churned accounts × ACV ÷ 12`, and expansion/downgrade use opening MRR. Reconcile realized movement through `churned customer ARPU = churned MRR ÷ churned customers` and its ratio to opening ARPU. Include both diagnostics in Forecast and churn CSV/PDF outputs.
- Predicted LTV is model-specific: B2C uses customer-weighted acquisition ARPU × gross margin ÷ effective monthly revenue churn; B2B uses `(ACV × gross margin) ÷ annual logo churn`. B2B payback is `blended CAC ÷ ((ACV ÷ 12) × gross margin)`.
...
- Monthly Forecast rows expand one at a time into a channel attribution table grouped as Baseline / Existing Business, Direct Response, Demand Gen, and Owned / Partner / Custom. Include launched enabled channels even when their current traffic is zero; cumulative channel customers and MRR must use the applicable B2C monthly churn or B2B annual logo-churn event plus expansion and downgrade assumptions, and category totals must reconcile to the parent month.
...
- Actual Blended CAC is model-specific. B2C includes one month of overhead alongside paid launch spend and affiliate commissions before dividing by attributed paid/partner customers. B2B treats overhead as annual and divides annual overhead plus the first 12 forecast months of paid spend plus affiliate commissions by all customers acquired in those 12 months.
