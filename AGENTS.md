...
- The public application has no accounts or database. The explicitly requested private BlueAlpha proposal uses a server-side PIN and a signed, expiring HttpOnly cookie; see README for its isolated Vercel route. The static site has one bounded Vercel `/api/lead` endpoint that validates agency leads, assigns a submission ID, and captures `lead_form_submitted` server-side in PostHog. It must never call Attio directly or expose server configuration through `VITE_` variables.
